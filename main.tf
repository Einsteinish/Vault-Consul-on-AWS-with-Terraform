...
data "template_file" "bastion" {
  template = "${file("${path.module}/templates/bastion-systemd.sh.tpl")}"
...
${data.template_file.bastion.rendered} # Configure Bastion 
EOF
}

data "template_file" "consul" {
  template = "${file("${path.module}/templates/consul-systemd.sh.tpl")}"
...
${data.template_file.consul.rendered} # Configure Consul 
EOF
}

data "template_file" "vault" {
  template = "${file("${path.module}/templates/vault-systemd.sh.tpl")}"
...
${data.template_file.consul_install.rendered} # Runtime install Consul in -dev mode
${data.template_file.vault_install.rendered} # Runtime install Vault in -dev mode
${data.template_file.vault.rendered} # Configure Vault 
EOF
}
